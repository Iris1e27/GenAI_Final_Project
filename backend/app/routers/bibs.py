...
from app.services.paper_processing import get_summary_by_entry, get_chart_by_entries, get_comparison_by_entries, get_categories_by_entries
...
async def generate_chart(data: dict):
    ...
    # 返回图表数据，这里返回一个示例响应
    return {"message": "Chart completed successfully", "paper_ids": paper_ids, "image_url": image_url}


@router.post("/categorize_themes")
async def categorize_themes(data: dict):
    # 从前端传入的数据中获取需要比较的 paper 的 ID
    paper_ids = data.get("paper_ids", [])
    print("paper_ids:"+str(paper_ids))
    ...
    result = get_categories_by_entries(entries)
    
    # 返回比较结果，这里返回一个示例响应
    return {"message": "Categorization completed successfully", "paper_ids": paper_ids, "result": result}
